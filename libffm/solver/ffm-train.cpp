...
Option parse_option(int argc, char **argv) {
  ...
  if (argc == 1) throw invalid_argument(train_help());
  ...
  for (; i < argc; i++) {
    if (args[i].compare("-t") == 0) {
      ...
      if (i == argc - 1) throw invalid_argument("need to specify eta after -r");
      i++;
...
int train_on_disk(Option opt) {
  ...
  if (!opt.va_path.empty()) ffm_read_problem_to_disk(opt.va_path, va_bin_path);
...
int main(int argc, char **argv) {
  ...
  if (opt.quiet) cout.setstate(ios_base::badbit);
