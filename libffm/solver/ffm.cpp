...
= nnz, len(P) = l + 1
Y: The label. len(Y) = l
R: Precomputed scaling factor to make the 2-norm of each instance to be 1.
len(R) = l
...
ffm_long get_wi_size(ffm_model &model) {
  return (ffm_long)model.n * 2 * kALIGN;
}
  ...
  for (ffm_node *N1 = begin; N1 != end; N1++) {
    ...
    for (ffm_node *N2 = N1 + 1; N2 != end; N2++) {
      ...
      if (do_update) {
        ...
        for (ffm_int d = 0; d < align0; d += kALIGN * 2) t += w1[d] * w2[d] * v;
      }
    }
  }
...
struct problem_on_disk {
  ...
  int load_block(int block_index) {
    if (block_index >= meta.num_blocks) assert(false);
...
void txt2bin(string txt_path, string bin_path) {
  ...
  while (fgets(line.data(), kMaxLineSize, f_txt)) {
    ...
    if (X.size() > (size_t)kCHUNK_SIZE) write_chunk();
  }
  write_chunk();
...
bool check_same_txt_bin(string txt_path, string bin_path) {
  ...
  if (f_bin.tellg() < (ffm_long)sizeof(disk_problem_meta)) return false;
  disk_problem_meta meta;
  f_bin.seekg(0, ios::beg);
  f_bin.read(reinterpret_cast<char *>(&meta), sizeof(disk_problem_meta));
  if (meta.hash1 != hashfile(txt_path, true)) return false;
  if (meta.hash2 != hashfile(txt_path, false)) return false;
...
ffm_model ffm_train_on_disk(string tr_path, string va_path,
  ...
  vector<ffm_float> prev_Wi(wi_size, 0);
...
ffm_float ffm_predict(ffm_node *begin, ffm_node *end, ffm_model &model) {
  ...
  if (model.normalization) {
    r = 0;
    for (ffm_node *N = begin; N != end; N++) r += N->v * N->v;
    r = 1 / r;
  }
